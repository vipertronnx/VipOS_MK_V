...
Quiet Mode suppresses viewer-triggered `overlay.alert`, `overlay.emit`, `sound.pickRandom`, and `sound.play` actions. It applies to chat, chat entries, Twitch events, redemptions, and reward events. Manual API and macro actions remain available.
